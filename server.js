...
const destDir = 'dist/';
const devScriptInFile = 'template/dev.user.js';
...
const devScriptOutFile = path.join(destDir, 'dev.user.js');
...
const devScriptContent = fs.readFileSync(devScriptInFile, 'utf8')
	.replace(/%PORT%/gmu, port.toString())
	.replace(/%NAME%/gmu, pkg.name)
	.replace(/%VERSION%/gmu, pkg.version);
			...
			setTimeout(async function () {
				...
				if (outFiles) {
					for (let i = 0; i < outFiles.length; i++) {
						const urlPath = outFiles[i].replace(/\\/gu, '/');
